...
const Legend = ({color, text}: {color: string, text: string}) => {
  return <div className="flex w-full h-[3rem]">
    <div className={`bg-black mr-4 w-8 h-8 rounded`}></div>
    <p> {text} </p>
  </div>
}
    ...
    <Container className="my-12 bg-gray-100 rounded-lg p-4">
      ...
      <div className="w-full flex">
        ...
        <div className="rw-1 mx-4 grid grid-rows-2 grid-cols-2 gap-4">
          {data?.statistics.map((stat, n) => <div className="flex flex-col h-[450px] justify-around items-center w-full" key={n}>
            <div className="relative w-full h-[200px]">
             {stat.meta.length > 0 && stat.meta.map((met, o) => <div className={`w-[100px] absolute border-2 border-black h-[100px] rounded-full origin-bottom left-1/2 bottom-0 ${o == 0 ? "circle1" : o == 1 ? "circle2" : "circle3"}`} key={o}>
              <p className="text-center mt-1"> {met} </p> 
             </div>)} 
            </div>
              <div className="w-4/5 h-[150px] mx-4">
                <div className="flex h-[100px] w-full justify-between items-center">
                  ...
                  <div className="h-full rw-1 flex flex-col items-end">
                    ...
                    <div className="flex h-[3rem] items-end">
                       <p className="text-6xl font-bold"> {stat.placed} </p> 
                ...
                <div className="w-full h-full flex justify-center items-center">
                    <div className="w-1/2 h-[200px]">
                      <Legend color={data?.layoutMeta.colors.dark as string} text={"intake"} />
                      <Legend color={data?.layoutMeta.colors.dark as string} text={"intake"} />
                      <Legend color={data?.layoutMeta.colors.dark as string} text={"intake"} />
                    </div>
                </div>  
